var app = angular.module('detailspropertyapp', []);
app.controller('PropertyDetailsController', function ($scope, $http, notify, blockUI, Upload, cfpLoadingBar, PropertyService) {
  // $scope.callCountUnApproved();
  //alert($scope.property_Id);
  ...
  fetchPropetyDetails($scope.property_Id);
  function fetchPropetyDetails(propertyId) {
    $http.post($scope.URL + "GetPropertyById", { 'propertyId': propertyId })

  .success(function (data, status, headers, config) {
    if (data.Id == 0) {
      swal({ title: "Error!", text: "No Property with this Id", type: "error", timer: 2000, showConfirmButton: false });
      //$window.location.href = '/Scripts/App/Property/PropertyUser.html';
      //   $location.path("/propertyuser");
      //$window.location.href = '/propertyuser';

    }
    $scope.PropertyId = data.Id;

    vm.Id = data.Id;
    vm.TitleAr = data.TitleAr;
    vm.Title = data.Title;
    vm.Description = data.Description;
    vm.DescriptionAr = data.DescriptionAr;
    vm.Address = data.Address;
    vm.AddressAr = data.AddressAr;
    vm.Price = data.Price;
    vm.BathroomNo = data.BathroomNo;
    vm.BedroomNo = data.BedroomNo;
    vm.RoomsNo = data.RoomsNo;
    vm.ReceptionNo = data.ReceptionNo;
    vm.Balacony = data.Balacony;
    vm.Garage = data.Garage;
    vm.Garden = data.Garden;
    vm.Pool = data.Pool;
    vm.Lift = data.Lift;
    vm.Area = data.Area;
    vm.Status = data.Status;
    vm.CreatedByUserName = data.CreatedByUserName;
    vm.CreatedBy = data.CreatedBy;
    var currencyId = parseInt(data.Currency);
    $scope.currencymodel = [{ id: currencyId }];

    var propertyTypeId = parseInt(data.PropertyType);
    $scope.propertytypemodel = [{ id: propertyTypeId }];

    var squareId = parseInt(data.Area);
    $scope.squaremodel = [{ id: squareId }];

    var contractTypeId = parseInt(data.ContractType);
    $scope.contracttypemodel = [{ id: contractTypeId }];

    vm.BedroomImages = data.ListImageBedroom;
    $scope.balaconyImages = data.ListImageBalacony;
    vm.PoolImages = data.ListImagePool;
    vm.ReceptionImages = data.ListImageReception;
    vm.GardenImages = data.ListImageGarden;
    vm.bathroomImages = data.ListImageBathroom;
    //vm.Late = data.Late;
    //vm.Long = data.Long;
    //vm.CreatedBy = data.CreatedBy;
 ...
 .error(function (data, status, headers, config) {
  ...
  $scope.UpdatePropertyStatus = function (property, propertystatus, propertyid) {
    // Get the reference to the block service.
    var myBlockUi = blockUI.instances.get('myBlockUI');
    ...
    //});
    cfpLoadingBar.start();
    property.ApprovedBy = $scope.UserId;
  ...
  function fetchAdminData() {
    ...
    $http.post($scope.URL + "FetchAdminPropertyByPageSize", { 'pageNumber': vm.currentPage, 'pageSize': vm.pageNumber })
    .success(function (data, status, headers, config) {
